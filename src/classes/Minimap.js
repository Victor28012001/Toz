...
export class Minimap {
  constructor(worldSize = 2000) {
    ...
    this.cachedInnerWidth = 0;
    this.cachedCompassWidth = 0;

    this.createMinimap();
    this.createTopCompass();
  ...
  setupResizeObserver() {
    const resizeObserver = new ResizeObserver((entries) => {
      for (const entry of entries) {
        if (entry.target === this.hudCompassInner) {
          this.cachedInnerWidth =
            entry.contentRect.width || this.hudCompassInner.scrollWidth;
        } else if (entry.target === this.hudCompass) {
          this.cachedCompassWidth = entry.contentRect.width;
        }
      }
    });

    resizeObserver.observe(this.hudCompass);
    resizeObserver.observe(this.hudCompassInner);
  }
  ...
  drawJetpackPickups() {
    ...
    if (window.world?.mapManager?.currentMap !== "greeble_map") return;
    if (!window.world?.jetpackPickups?.length) {
      console.log("No jetpack pickups to draw");
      return;
    }

    const pickups = window.world.jetpackPickups;

    pickups.forEach((pickup, idx) => {
      // ✅ ADD 'idx' as the second parameter
      if (pickup.collected) {
        return;
      }

      const x = this.worldToMapX(pickup.position.x, this.player.position.x);
      const y = this.worldToMapY(pickup.position.z, this.player.position.z);
      ...
      const maxR = this.mapSize / 2 - 4;

      if (distFromCenter > maxR) {
        // Draw arrow at edge pointing toward pickup
        const angle = Math.atan2(dy, dx);
        const ex = cx + Math.cos(angle) * maxR;
        const ey = cy + Math.sin(angle) * maxR;

        const size = 5 * this.uiScale;
        this.ctx.save();
      ...
      // Draw glowing blue dot for pickup
      const r = 4 * this.uiScale;

      // Outer glow
      this.ctx.beginPath();
      this.ctx.arc(x, y, r + 2, 0, Math.PI * 2);
      this.ctx.fillStyle = "rgba(0, 170, 255, 0.25)";
      this.ctx.fill();
      ...
      this.ctx.font = `${8 * this.uiScale}px Arial`;
      this.ctx.textAlign = "center";
      this.ctx.textBaseline = "middle";
      this.ctx.fillText("🚀", x, y - r - 5);
    });
  }
  ...
  updateTopCompass() {
    ...
    if (this.cachedInnerWidth === 0 || this.cachedCompassWidth === 0) {
      return;
    }

    // Calculate shift using cached values (NO READS)
    const shift = (deg / 360) * this.cachedInnerWidth;

    // USE TRANSFORM: This is much faster than setting .style.left
    const translateX = -shift + this.cachedCompassWidth / 2;
    this.hudCompassInner.style.transform = `translateX(${translateX}px)`;
  }
