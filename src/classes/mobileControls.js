...
export class MobileControls {
  ...
  createActionButtons() {
    ...
    // Jump button (bottom-right of shoot)
    const jumpBtn = this.createImageButton(
      "jump",
      "/assets/ui/jump.png",
      smallSize,
    );
    jumpBtn.style.position = "absolute";
    jumpBtn.style.left = "102px";
    jumpBtn.style.bottom = "5px";
    buttonContainer.appendChild(jumpBtn);

    // Sprint toggle (top-right area)
    const sprintBtn = this.createImageButton(
      "sprint",
      "/assets/ui/sprint.png",
      smallSize,
    );
    sprintBtn.style.position = "absolute";
    sprintBtn.style.left = "85px";
    sprintBtn.style.top = "45px";
    this.addRunningIndicator(sprintBtn);
    buttonContainer.appendChild(sprintBtn);

    // Aim toggle (above shoot button)
    const aimBtn = this.createImageButton("aim", "/assets/ui/aim.png", 60);
    aimBtn.style.position = "absolute";
    aimBtn.style.left = "10px";
    aimBtn.style.bottom = "105px";
    buttonContainer.appendChild(aimBtn);
  ...
  //         return;
  //       case "shoot":
  //         // Only shoot when aiming and button is pressed
  //         if (pressed && this.controls && this.controls.isAiming) {
  //           this.controls.player.startShooting();
  //         } else if (!pressed && this.controls && this.controls.isAiming) {
  //           this.controls.player.stopShooting();
  //         }
  //         return;
  //       case "weaponWheel":
  ...
  //         if (this.controls) {
  //           const isGreebleMap =
  //             this.controls.world?.mapManager?.currentMap === "greeble_map";

  //           if (
  //             isGreebleMap &&
  //             this.controls.jetpackActive &&
  //             this.controls.jetpackFuel > 0
  //           ) {
  //             // On greeble map with jetpack: jetpack on press
  //             this.controls.isJetpacking = pressed;
  //             if (pressed) {
  //               this.showNotification("JETPACK ENGAGED!", "#00aaff");
  //             } else {
  //               this.showNotification("JETPACK STOPPED", "#ffaa00");
  //             }
  //           } else if (
  //             !isGreebleMap &&
  //             pressed &&
  //             !this.controls.jumpCooldown
  //           ) {
  //             // On shooting range: normal jump
  //             if (this.controls.isOnObject || this.controls.canJump) {
  //               this.controls.velocity.y = 1.1;
  ...
  // Setup HUD button interactions
  setupHUDInteractions() {
    // Wait for ModernHUD to be created
    const checkHUD = setInterval(() => {
      const weaponSlot = document.getElementById("hud-weapon-slot");
      const grenadeSlot = document.querySelector(".grenade-slot");
      const bagSlot = document.querySelector(".bag-slot");
      const secondarySlot = document.querySelector(".secondary-slot");

      if (weaponSlot && grenadeSlot && bagSlot && secondarySlot) {
        clearInterval(checkHUD);
        this._bindHUDEvents(weaponSlot, grenadeSlot, bagSlot, secondarySlot);
      }
    }, 500);
  }

  _bindHUDEvents(weaponSlot, grenadeSlot, bagSlot, secondarySlot) {
    // Weapon slot - tap to open weapon wheel
    weaponSlot.style.pointerEvents = "auto";
    weaponSlot.addEventListener("touchstart", (e) => {
      e.preventDefault();
      e.stopPropagation();
      this.showWeaponWheel();
    });

    // Secondary weapon slot - tap to switch to next weapon
    secondarySlot.style.pointerEvents = "auto";
    secondarySlot.addEventListener("touchstart", (e) => {
      e.preventDefault();
      e.stopPropagation();
      if (this.controls && this.controls.player) {
        const nextIndex =
          ...
          this.controls.player.weapons.length;
        this.controls.player.switchWeapon(nextIndex, true);
        this.showNotification("Weapon Switched", "#ffaa00");
      }
    });

    // Bag/Backpack slot
    bagSlot.style.pointerEvents = "auto";
    this._inventoryOpen = false;

    // Add pulsing dot for collectibles
    const collectDot = document.createElement("div");
    collectDot.className = "collect-dot";
    collectDot.style.cssText = `
      position: absolute;
      top: 4px;
      right: 4px;
      width: 10px;
      ...
      background: #ff4444;
      display: none;
      animation: collectPulse 1s infinite;
    `;
    bagSlot.appendChild(collectDot);

    bagSlot.addEventListener("touchstart", (e) => {
      e.preventDefault();
      e.stopPropagation();

      // If there's a collectible nearby, collect it
      if (window.roomManager) {
        window.roomManager.handleInteraction();
      }

      // Request and open inventory
      if (this.controls?.player?.socket) {
        const handler = (data) => {
          this.controls.player.socket.off("inventoryUpdate", handler);
          if (window.modernHUD) {
            window.modernHUD.toggleInventory(
              data.inventory,
              this.controls.player.socket,
            );
          }
        };
        this.controls.player.socket.on("inventoryUpdate", handler);
        this.controls.player.socket.emit("requestInventory");
      }
    });

    // Show/hide collect dot based on proximity to interactable objects
    // This should be updated in your game loop
    this.updateCollectDot = (canInteract) => {
      if (collectDot) {
        collectDot.style.display = canInteract ? "block" : "none";
      }
    };

    // Grenade slot - hold to aim, release to throw
    grenadeSlot.style.pointerEvents = "auto";
    let grenadeTimeout = null;

    grenadeSlot.addEventListener("touchstart", (e) => {
      e.preventDefault();
      e.stopPropagation();
      if (this.controls && this.controls.isAiming) {
        // Start grenade aim
        this.controls.isGrenadeAiming = true;
        this.controls.player.playAnimation("idle", true);
        this.showNotification("Grenade Aiming... Release to throw", "#ff6600");
      }
    });

    grenadeSlot.addEventListener("touchend", (e) => {
      e.preventDefault();
      e.stopPropagation();
      if (this.controls && this.controls.isGrenadeAiming) {
        this.controls.isGrenadeAiming = false;
        this.controls.throwGrenade();
        this.showNotification("Grenade Thrown!", "#ff6600");
      }
    });

    // Add keyframe animation for collect dot
    if (!document.getElementById("mobile-hud-styles")) {
  ...
  handleButtonStart(e) {
    ...
    if (action === "sprint") {
      ...
      // Jump button also handles jetpack on greeble map
      this.buttons.jump = true;
      window.dispatchEvent(
        new CustomEvent("mobileButtonPress", {
          detail: { action: "jump", pressed: true, toggle: false },
        }),
      );

      // Visual feedback for jetpack when on greeble map with active jetpack
      if (
        this.controls &&
        this.controls.world?.mapManager?.currentMap === "greeble_map" &&
        this.controls.jetpackActive &&
        ...
        button.style.background = "rgba(0, 200, 255, 0.9)";
        button.style.boxShadow = "0 0 15px rgba(0,170,255,0.8)";
      }
  ...
  handleButtonEnd(e) {
    ...
    if (action === "jump") {
      // Release jump/jetpack
      this.buttons.jump = false;
      window.dispatchEvent(
        new CustomEvent("mobileButtonPress", {
          detail: { action: "jump", pressed: false, toggle: false },
        }),
      );
      button.style.background = "rgba(0, 0, 0, 0.65)";
      button.style.boxShadow = "0 4px 12px rgba(0, 0, 0, 0.4)";
    } else if (action !== "sprint" && action !== "aim") {
