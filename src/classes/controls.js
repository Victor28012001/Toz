...
export class PointerLockControlsCustom {
  constructor(camera, player, world) {
    ...
    this.fpsWeapon = null; // Duplicate weapon attached to camera
    this.fpsWeaponOffset = new THREE.Vector3(0.3, -0.2, -0.5); // Position relative to camera (right, down, forward)
    this.fpsWeaponRotation = new THREE.Vector3(0, 0, 0); // Rotation offset

    this.fpsWeaponOffset = new THREE.Vector3(0.35, -0.25, -0.9);
    this.fpsWeaponRotation = new THREE.Vector3(0, Math.PI, 0);
    this.fpsWeaponIdleSway = { x: 0, y: 0, time: 0 };
    this.fpsWeaponRecoil = 0;
    this.fpsWeaponRecoilReset = 0;
    this.fpsWeaponBobbing = { x: 0, y: 0, time: 0 };
  ...
  reload() {
    ...
    // Trigger FPS weapon reload animation
    if (this.cameraMode === "fps") {
      this.addFPSWeaponReload();
    }
  ...
  createFPSWeapon() {
    ...
    // Remove existing FPS weapon
    if (this.fpsWeapon) {
      this.camera.remove(this.fpsWeapon);
      this.fpsWeapon = null;
    }
    ...
    // Set offset - adjust these values to position the weapon correctly
    this.fpsWeaponOffset = new THREE.Vector3(0.35, -0.45, -1);

    // Clone the weapon model
    const weaponClone = currentWeapon.model.clone();
    weaponClone.name = "fps_weapon";

    // Position relative to camera
    weaponClone.position.copy(this.fpsWeaponOffset);

    // Store original rotation for FPS view (facing forward)
    this.fpsWeaponBaseRotation = new THREE.Euler(
      this.fpsWeaponRotation.x,
      this.fpsWeaponRotation.y, // 180 degrees to face forward
      this.fpsWeaponRotation.z,
    );
    weaponClone.rotation.copy(this.fpsWeaponBaseRotation);

    // Add to camera
    this.camera.add(weaponClone);
    this.fpsWeapon = weaponClone;

    console.log("✅ FPS weapon attached to camera");
  }

  updateFPSWeapon() {
    if (!this.fpsWeapon) return;

    // Update weapon position/rotation if needed (for recoil, aim down sights, etc.)
    const currentWeapon = this.player.weapons[this.player.currentWeaponIndex];
    if (currentWeapon && currentWeapon.isAiming) {
      // Optional: adjust for aiming down sights
      this.fpsWeapon.position.set(
        this.fpsWeaponOffset.x * 0.5,
        this.fpsWeaponOffset.y * 0.3,
        this.fpsWeaponOffset.z * 0.8,
      );
    } else {
      this.fpsWeapon.position.copy(this.fpsWeaponOffset);
    }
  }

  // Update FPS weapon animations (call this in update method)
  updateFPSWeaponAnimation(delta, isMoving, isRunning) {
    if (this.isReloading || !this.fpsWeapon || this.cameraMode !== "fps")
      return;

    // Update idle sway time - slower
    this.fpsWeaponIdleSway.time += delta * 0.05;

    // Idle sway - gentle floating movement (position)
    const idleSwayX = Math.sin(this.fpsWeaponIdleSway.time * 0.8) * 0.005;
    const idleSwayY = Math.cos(this.fpsWeaponIdleSway.time * 0.6) * 0.003;

    // IDLE SWAY ROTATION - Side to side (Y axis) instead of back and forth (X/Z)
    const idleSwayRotY = Math.sin(this.fpsWeaponIdleSway.time * 0.7) * 0.01;
    const idleSwayRotX = Math.sin(this.fpsWeaponIdleSway.time * 0.9) * 0.005;
    ...
    if (isMoving && !this.isInWater) {
      this.fpsWeaponBobbing.time += delta * (isRunning ? 0.2 : 0.1);

      const bobSpeed = isRunning ? 1.5 : 1.0;
      const bobAmountX = isRunning ? 0.02 : 0.012;
      const bobAmountY = isRunning ? 0.028 : 0.016;

      bobX = Math.sin(this.fpsWeaponBobbing.time * bobSpeed) * bobAmountX;
      bobY =
        Math.abs(Math.sin(this.fpsWeaponBobbing.time * bobSpeed * 1.5)) *
        bobAmountY;
      bobRotY = Math.sin(this.fpsWeaponBobbing.time * bobSpeed) * 0.008;
      bobRotX = Math.sin(this.fpsWeaponBobbing.time * bobSpeed * 1.2) * 0.004;
      ...
      this.fpsWeaponBobbing.time = 0;
    }

    // Recoil recovery
    if (this.fpsWeaponRecoil > 0) {
      this.fpsWeaponRecoil -= delta * 5;
      if (this.fpsWeaponRecoil < 0) this.fpsWeaponRecoil = 0;
    }

    if (this.fpsWeaponRecoilReset > 0) {
      this.fpsWeaponRecoilReset -= delta;
      if (this.fpsWeaponRecoilReset <= 0) {
        this.fpsWeaponRecoil = 0;
      }
    }

    // Apply position animations - RECOIL NOW ON Z AXIS (forward/backward)
    const targetX = this.fpsWeaponOffset.x + idleSwayX + bobX;
    const targetY = this.fpsWeaponOffset.y + idleSwayY + bobY;
    const targetZ = this.fpsWeaponOffset.z + this.fpsWeaponRecoil * 0.15; // Positive Z = move toward camera (pull back)

    // Smooth interpolation for position
    this.fpsWeapon.position.x += (targetX - this.fpsWeapon.position.x) * 0.2;
    this.fpsWeapon.position.y += (targetY - this.fpsWeapon.position.y) * 0.2;
    this.fpsWeapon.position.z += (targetZ - this.fpsWeapon.position.z) * 0.3; // Faster response for recoil

    // Apply rotation animations - RECOIL NOW ROTATES X AXIS (tilt up)
    const finalRotY = this.fpsWeaponBaseRotation.y + idleSwayRotY + bobRotY;
    const finalRotX =
      this.fpsWeaponBaseRotation.x +
      idleSwayRotX +
      bobRotX +
      this.fpsWeaponRecoil * 0.08; // Tilt up on X axis
    const finalRotZ = this.fpsWeaponBaseRotation.z;

    this.fpsWeapon.rotation.x = finalRotX;
    this.fpsWeapon.rotation.y = finalRotY;
    this.fpsWeapon.rotation.z = finalRotZ;
  }

  // Add shooting recoil to FPS weapon
  addFPSWeaponRecoil() {
    if (this.cameraMode !== "fps" || !this.fpsWeapon) return;

    // Add recoil - pushes weapon BACK and UP
    this.fpsWeaponRecoil = Math.min(1.0, this.fpsWeaponRecoil + 0.35);
    this.fpsWeaponRecoilReset = 0.2;

    // Immediate visual feedback
    if (this.fpsWeapon) {
      this.fpsWeapon.position.z += 0.08;
      this.fpsWeapon.rotation.x -= 0.04;
    }

    if (this.world?.crosshair3D) {
      this.world.crosshair3D.fireFeedback();
    }
  }

  addFPSWeaponReload() {
    if (this.cameraMode !== "fps" || !this.fpsWeapon || this.isReloading)
      return;

    this.isReloading = true;

    // Store original position and rotation
    const originalX = this.fpsWeaponOffset.x;
    const originalY = this.fpsWeaponOffset.y;
    const originalZ = this.fpsWeaponOffset.z;
    const originalRotX = this.fpsWeaponBaseRotation.x;
    const originalRotY = this.fpsWeaponBaseRotation.y;
    const originalRotZ = this.fpsWeaponBaseRotation.z;
    ...
    const animateReload = () => {
      ...
      let progress = Math.min(1, elapsed / reloadDuration);

      if (progress < 0.55) {
        let t = progress / 0.55;

        // Cubic ease in for more dramatic effect
        const easeIn = t * t;

        // Rotate weapon sideways (Z rotation) and tilt up (X rotation)
        const rotZ = originalRotZ + 0.8 * easeIn; // Dramatic sideways tilt
        const rotX = originalRotX - 0.5 * easeIn; // Tilt up
        const rotY = originalRotY + 0.3 * easeIn; // Rotate inward

        // Move weapon down and to the side
        const offsetX = originalX + 0.6 * easeIn;
        const offsetY = originalY - 0.5 * easeIn;
        const offsetZ = originalZ + 0.3 * easeIn;

        this.fpsWeapon.position.set(offsetX, offsetY, offsetZ);
        this.fpsWeapon.rotation.x = rotX;
        this.fpsWeapon.rotation.y = rotY;
        this.fpsWeapon.rotation.z = rotZ;

        requestAnimationFrame(animateReload);
      } else {
        // Return to original with overshoot
        ...
        const returnX = originalX + 0.6 * (1 - easeOut);
        const returnY = originalY - 0.5 * (1 - easeOut);
        const returnZ = originalZ + 0.3 * (1 - easeOut);

        const returnRotZ = originalRotZ + 0.8 * (1 - easeOut) - overshoot * 0.3;
        const returnRotX = originalRotX - 0.5 * (1 - easeOut) + overshoot * 0.2;
        const returnRotY = originalRotY + 0.3 * (1 - easeOut);

        this.fpsWeapon.position.set(returnX, returnY, returnZ);
        this.fpsWeapon.rotation.x = returnRotX;
        this.fpsWeapon.rotation.y = returnRotY;
        this.fpsWeapon.rotation.z = returnRotZ;
        ...
        if (progress < 1) {
          ...
          // Final exact reset
          this.fpsWeapon.position.copy(this.fpsWeaponOffset);
          this.fpsWeapon.rotation.copy(this.fpsWeaponBaseRotation);
          this.isReloading = false;
        }
      }
    };
  ...
  setFPSWeaponAiming(aiming) {
    if (this.cameraMode !== "fps" || !this.fpsWeapon) return;

    const targetX = aiming
      ? this.fpsWeaponOffset.x * 0.4
      : this.fpsWeaponOffset.x;
    const targetY = aiming
      ? this.fpsWeaponOffset.y * 0.5
      : this.fpsWeaponOffset.y;
    const targetZ = aiming
      ? this.fpsWeaponOffset.z * 0.7
      : this.fpsWeaponOffset.z;

    // Smooth transition to ADS position
    const speed = 0.15;
    this.fpsWeapon.position.x += (targetX - this.fpsWeapon.position.x) * speed;
    this.fpsWeapon.position.y += (targetY - this.fpsWeapon.position.y) * speed;
    this.fpsWeapon.position.z += (targetZ - this.fpsWeapon.position.z) * speed;
  }
  ...
  update(delta) {
    ...
    if (this.player.throwInProgress || this.isGrenadeAiming) {
      ...
      // Update FPS weapon animation
      if (this.cameraMode === "fps") {
        this.updateFPSWeaponAnimation(delta, isMoving, isRunning);
      }
        ...
        // Normal ground movement
          ...
          if (isMoving) {
            if (this.cameraMode === "thirdPerson") {
              // Third person: use walk1/run1 (original rifle animations)
              targetAnimation = isRunning ? "run1" : "walk1";
            } else {
              // Shoulder/FPS: use walk/run (forward-facing animations)
  ...
  //     // When aiming, smoothly rotate player to face camera direction
  ...
  //     eyePos.y += 16; // Eye level height

  //     const pullDist = 18; // Distance behind player
  //     const backX = Math.sin(camYaw) * pullDist;
  //     const backZ = Math.cos(camYaw) * pullDist;

  //     const sideOffset = 6; // Right shoulder offset
  ...
  //     // Make camera look at where the player is aiming
  //     const lookDist = 80;
  ...
  //   } else {
  //     // --- Standard third-person free orbit ---
  ...
  updateCameraPosition(delta) {
    ...
    // FPS Camera Mode - First-person from head bone
    if (this.cameraMode === "fps") {
      if (!this.headBone && this.player?.modelLoaded) {
        this.findHeadBone();
      }
      ...
      if (this.headBone) {
        ...
        headPos = this.playerObj.position.clone();
        headPos.y += 16;
      }

      const camYaw = this.cameraRotation.x;

      // Rotate player to face camera direction in FPS mode
      let angleDiff = camYaw - this.playerObj.rotation.y;
      angleDiff = Math.atan2(Math.sin(angleDiff), Math.cos(angleDiff));
      this.playerObj.rotation.y += angleDiff * 0.15;

      // Small forward offset to avoid seeing inside the head
      const forward = new THREE.Vector3(0, 0, -1).applyEuler(
        new THREE.Euler(0, camYaw, 0),
      );
      const cameraPos = headPos
        .clone()
      ...
      // Look at aim target
      const pitchNorm = (this.cameraRotation.y - 0.1) / (this.PI_2 - 0.2);
      const pitchAngle = (pitchNorm - 0.3) * (Math.PI * 0.6);
      const lookDist = 80;
      const aimTarget = new THREE.Vector3(
        this.playerObj.position.x -
    ...
    // SHOULDER MODE (over-the-shoulder aiming) - rotates player
    if (this.isAiming || this.cameraMode === "shoulder") {
      const camYaw = this.cameraRotation.x;

      // 👇 ADD THIS OFFSET - rotates player slightly right to center the camera view
      const SHOULDER_OFFSET_ANGLE = 0.3; // Adjust this value to fine-tune (radians)
      const adjustedYaw = camYaw - SHOULDER_OFFSET_ANGLE; // Positive = rotate player to the right

      // Rotate player to face the ADJUSTED direction
      let angleDiff = adjustedYaw - this.playerObj.rotation.y;
      angleDiff = Math.atan2(Math.sin(angleDiff), Math.cos(angleDiff));
      this.playerObj.rotation.y += angleDiff * 0.15;
    ...
    // THIRD PERSON MODE - Free orbit, does NOT rotate player
    // Standard third-person free orbit - player stays facing movement direction
    const horizontalDistance =
      this.currentCameraDistance * Math.cos(this.cameraRotation.y);
    const verticalDistance =
      this.currentCameraDistance * Math.sin(this.cameraRotation.y);
    ...
    // Look at the player (not rotating the player)
    const lookAtPos = this.playerObj.position
      .clone()
  ...
  //     }
  //     // In third person mode, aiming is disabled
  //     this.isAiming = false;
  //     this.targetFOV = this.normalFOV;
  //   }
  ...
  cycleCameraMode() {
    ...
    if (this.cameraMode === "fps") {
      ...
      // Create FPS weapon attached to camera
  ...
  //     new THREE.Vector3(
  //       this.playerObj.position.x,
  //       this.playerObj.position.y + 1,
  //       this.playerObj.position.z,
  //     ),
  //     new THREE.Vector3(0, -1, 0),
  //   );

  //   // Filter out Boundary meshes from ground detection
  ...
  checkCollisions() {
    ...
    if (closestHit && closestDistance < 1.5) {
      ...
      if (this.player && this.player.currentAnimation === "falling") {
        ...
        if (isMoving) {
          if (this.cameraMode === "thirdPerson") {
            this.player.playAnimation(this.isRunning ? "run1" : "walk1", true);
          } else {
  ...
  //   // Skip building collisions entirely when in water
  //   if (this.isInWater) return;
  //   if (!this.world.buildingMeshes || !this.playerCollider) return;

  //   // Skip frames
  //   this.collisionFrameCounter = (this.collisionFrameCounter || 0) + 1;
  //   if (this.collisionFrameCounter % this.collisionFrameSkip !== 0) return;

  //   const playerPos = this.playerObj.position;

  //   // ✅ ONLY check meshes within 80 units — cull distant ones
  ...
  //     // Distance cull — skip if mesh bounding sphere is far away
  //     if (!mesh.geometry.boundingSphere) mesh.geometry.computeBoundingSphere();
  //     const meshWorldPos = new THREE.Vector3();
  //     mesh.getWorldPosition(meshWorldPos);
  //     const dist = playerPos.distanceTo(meshWorldPos);
  ...
  //   if (this.isMobile) {
  //     if (this.collisionFrameCounter === undefined) {
  //       this.collisionFrameCounter = 0;
  //     }
  //     this.collisionFrameCounter++;
  //     if (this.collisionFrameCounter % 2 !== 0) {
  //       return;
  //     }
  //     if (this.collisionFrameCounter > 1000) {
  //       this.collisionFrameCounter = 0;
  //     }
  //   }
  ...
  //       // Skip water meshes completely
  //       if (mesh.userData?.isWater || mesh.name === "Object_69") {
  //         continue;
  //       }
  //       // ✅ SKIP BRANCH MESHES - Check by name or material
  //       if (mesh.name === "Object_61" || mesh.name === "Object_37") {
  //         continue; // Skip branch/foliage meshes entirely
  //       }

  //       // ✅ SKIP WATER MESHES - Don't apply collision response for water
  //       if (mesh.userData?.isWater) {
  //         continue; // Skip normal collision response for water
  //       }

  //       // Also check by userData flag (set in MapManager)
  //       if (mesh.userData?.skipCollision) {
  //         continue;
  //       }

  //       // Check by material name for branches
  //       if (mesh.material) {
  ...
  //             if (matName === "branch" || matName === "drytree_texture") {
  //               isBranch = true;
  //               break;
  //             }
  //           }
  //         }
  //         if (isBranch) {
  //           continue; // Skip branch by material
  //         }
  //       }
  ...
  //       // const inverseMat = new THREE.Matrix4().copy(matrixWorld).invert();
  ...
  //           // ✅ CHECK BY MESH NAME - JUST LIKE THE WORKING PLAYTEST CODE
  //           const normal = tri.getNormal(new THREE.Vector3());
  //           normal.applyQuaternion(mesh.quaternion);
  //           normal.normalize();

  //           // ✅ IGNORE TOP SURFACE OF "Boundary" MESH (same logic as playtest)
  //           if (mesh.name === "Boundary" && normal.y > 0.7) {
  //             return; // Skip collision - allow falling through the top
  //           }
  ...
  startAiming() {
    ...
    // For FPS mode, trigger ADS animation
    if (this.cameraMode === "fps") {
      this.setFPSWeaponAiming(true);
    }
  ...
  stopAiming() {
    ...
    // For FPS mode, reset ADS animation
    if (this.cameraMode === "fps") {
      this.setFPSWeaponAiming(false);
    }
