...
export class MobileControls {
  ...
  setupKeyboardProxy() {
    window.addEventListener("mobileButtonPress", (e) => {
      ...
      switch (action) {
        ...
        case "collect":
          if (pressed) {
            // Dispatch a custom event that the RoomManager can listen for
            window.dispatchEvent(new CustomEvent("collectItem"));

            // Also try to directly trigger the key press if RoomManager is listening for keyboard
  ...
  handleButtonStart(e) {
    ...
    // Special handling for sprint button - toggle instead of hold
    if (action === "sprint") {
      // Toggle sprint state
      const newSprintState = !this.buttons.sprint;
      this.buttons.sprint = newSprintState;

      // Update button visual to show active state
      if (newSprintState) {
      ...
      // Show notification for collect button
      ...
      // For other buttons, dispatch press event
  ...
  handleButtonEnd(e) {
    ...
    // Don't release sprint button - it's a toggle
    if (action !== "sprint") {
