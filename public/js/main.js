...
window.onload = () => {
  ...
  socket.on("initSelf", (data) => {
    ...
    // The player position should already be set by the server
    // But if you want to ensure they're at a spawn point:
    if (window.spawnPoints && window.spawnPoints.length > 0) {
      // Find which spawn point matches the player's position
      ...
      if (matchingSpawn) {
        console.log(
          `Player spawned at spawn point ${window.spawnPoints.indexOf(matchingSpawn)}`,
        );
      }
    }

    document.addEventListener("keydown", (e) => {
      if (e.code === "KeyF" && world && world.controls) {
  ...
  socket.on("spawnPoints", (spawnPositions) => {
    // Store spawn points globally or in your world
    window.spawnPoints = spawnPositions;

    // Create boxes at these positions
...
function startGame() {
  ...
  // NEW: expose roomManager globally so Player.js can reference it
  window.roomManager = roomManager;
}

// NEW: ammo display (ammo / max  ×reserves)
function createAmmoHUD() {
  ...
  `;
  hud.innerHTML = `
    <span style="color:#aaa;font-size:12px" id="ammo-weapon-name">—</span>
    <span style="font-size:22px;font-weight:bold" id="ammo-current">—</span>
    <span style="color:#666;font-size:16px">/</span>
    <span style="color:#888;font-size:16px" id="ammo-max">—</span>
    <span style="font-size:13px;min-width:36px;text-align:right" id="ammo-reserves"></span>
  `;
